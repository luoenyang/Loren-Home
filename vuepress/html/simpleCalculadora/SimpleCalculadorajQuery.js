...
jQuery.fn.extend({
  Calculadora: function (op) {
    ...
    defaults = {
      TituloHTML:
        '<a class="btn-block btn3d btn btn-success" href="http://develoteca.com" target="_blank">Calculadora</a>',
      Botones: [
        "7",
        "8",
        "9",
    ...
    $(LaCalculadora).html(
      '<div class="panel panel-primary btn-block calculadoraBase  mtop">' +
        op.TituloHTML +
        '<div class="panel-body"><div class="col-md-12" style="margin-bottom: 10px;">' +
        $(LaCalculadora).html() +
        "</div></div> </div>"
    );
    ...
    $("." + NombreBotonesClase).click(function () {
      ...
      if (vTecla == "=") {
        var value = salida.val();
        value = value.replace(/\u200B/g, "");
        salida.val(eval(value));
      } else {
        if (salida.val() == 0) {
